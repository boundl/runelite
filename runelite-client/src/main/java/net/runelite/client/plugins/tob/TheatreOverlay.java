...
public class TheatreOverlay extends Overlay {
			...
			if (config.VerzikYellow()) {
				for (WorldPoint p : plugin.getVerzik_YellowTiles()) {
					...
					if (yellowBall != null) {
						...
						Point canvasPoint = Perspective.getCanvasTextLocation(client, graphics,
								LocalPoint.fromWorld(client, p), countdownStr, 0);
	...
	private void renderNpcOverlay(Graphics2D graphics, NPC actor, Color color, int outlineWidth, int outlineAlpha,
			int fillAlpha) {
		int size = 1;
	...
	private List<WorldPoint> getHitSquares(WorldPoint npcLoc, int npcSize, int thickness, boolean includeUnder) {
		...
		List<WorldPoint> big = new WorldArea(npcLoc.getX() - thickness, npcLoc.getY() - thickness,
				npcSize + (thickness * 2), npcSize + (thickness * 2), npcLoc.getPlane()).toWorldPointList();
		if (!includeUnder) {

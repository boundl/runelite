...
@PluginDescriptor(name = "Theatre of Blood", description = "All-in-one plugin for Theatre of Blood", tags = {
		"ToB" }, enabledByDefault = true)
	...
	public void onGameTick(GameTick event) {
				...
				if (!BlackTilesUnderworld.isEmpty() && !RedTilesUnderworld.isEmpty() && GridPath.isEmpty()) {
					...
					for (WorldPoint p : RedTilesUnderworld) {
						...
						if (!((RedTilesUnderworld.contains(pN) && RedTilesUnderworld.contains(pS))
							...
							if (!messageSent) {
								// client.addChatMessage(ChatMessageType.SERVER, "", "Maze path acquired.",
								// null);
								messageSent = true;
							}
						}
		...
		if (runXarpus) {
			...
			if (Xarpus_Stare) {
				...
				if (Xarpus_NPC.getAnimation() == ANIMATION_ID_XARPUS
						&& Xarpus_previousAnimation != ANIMATION_ID_XARPUS) {
